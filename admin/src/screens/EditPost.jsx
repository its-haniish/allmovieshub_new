...
const EditPost = () => {
    ...
    const getPostInfo = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${process.env.REACT_APP_BASE_URL}/fetch-one`, {
                method: 'POST',
            ...
            const { title, featured, author, slug: postSlug, contents, keywords, description } = result;
            setFormData({ title, featured, author, slug: postSlug, keywords, description });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${process.env.REACT_APP_BASE_URL}/update`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            <form className="flex flex-col w-full px-2 justify-start items-center gap-3 overflow-scroll" onSubmit={!loading ? handleSubmit : e => e.preventDefault()}>
                ...
                <div className="flex flex-col justify-start items-center w-full mt-2 h-fit">
                    ...
                    <textarea
                        ...
                        required
                    />
                    <p className='text-red-600 text-sm'>Keywords should be seperated by comma.</p>
                </div>
